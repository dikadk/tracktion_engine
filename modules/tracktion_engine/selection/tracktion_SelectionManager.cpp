...
namespace tracktion_engine
{
    ...
    static bool canSelectAtTheSameTime(const SelectableList &selected, Selectable &newItem)
    {
        ...
        for (int i = 0; i < selected.size(); ++i)
        {
            ...
            if (auto currentClass = selected.getSelectableClass(i))
                if (!(currentClass->canClassesBeSelectedAtTheSameTime(newItemClass) && currentClass->canObjectsBeSelectedAtTheSameTime(*s, newItem)))
                    return false;
        }
    ...
    void SelectionManager::select(Selectable &s, bool addToCurrentSelection)
    {
        ...
        {
            addToCurrentSelection = addToCurrentSelection && canSelectAtTheSameTime(selected, s);
